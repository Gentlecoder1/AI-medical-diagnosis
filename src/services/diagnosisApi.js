...
/**
 * Submit diagnosis data to AI service (OpenAI GPT)
...
export const submitDiagnosisData = async (formData) => {
  try {
    // Format symptoms for GPT analysis
    const symptomsDescription = `
Patient presents with:
- Hardness: ${formData.hardness}
- Pain severity: ${formData.painSeverity}/10
- Location: ${formData.position}
- Duration: ${formData.duration}
${
  formData.additionalSymptoms
    ? `- Additional symptoms: ${formData.additionalSymptoms}`
    : ""
}
    `.trim();
    ...
    const patientInfo = {
      ...
      hasImage: !!formData.medicalImages,
    };

    // Call OpenAI API through our Next.js API route
